...
gallery/ks-timeline/1.0/base
gallery/ks-timeline/1.0/background
gallery/ks-timeline/1.0/toolbar
gallery/ks-timeline/1.0/main-content
gallery/ks-timeline/1.0/dd
gallery/ks-timeline/1.0/config
gallery/ks-timeline/1.0/control
gallery/ks-timeline/1.0/index
...
KISSY.add('gallery/ks-timeline/1.0/base', function(S){
  function Base(){
...
KISSY.add('gallery/ks-timeline/1.0/background', function(S, Base, TT){
  var $ = S.all;
  var ATTRS = {
    html: {
      /*
...
KISSY.add('gallery/ks-timeline/1.0/toolbar', function(S, Base){
  var $ = S.all;
...
KISSY.add('gallery/ks-timeline/1.0/main-content', function(S, Base, TT, Anim){
  var $ = S.all;
  ...
  var o = {
    ...
    ,adjustStyle: function(){
      ...
      //时间标尺
      // self.get('timeRuler').width(trackConfig.realWidth + 20*Math.abs(trackConfig.spacer)).css('left', 0-Math.abs(2*trackConfig.spacer));
      self.get('timeRuler').width(9999999).css('left', 0-Math.abs(2*trackConfig.spacer));
    }
...
/**
  * 普通元素拖拽
  */
KISSY.add('gallery/ks-timeline/1.0/dd', function(S, Base){
  var $ = S.all, D = S.DOM;
 ...
 */
KISSY.add('gallery/ks-timeline/1.0/config', function(S){
  ...
  var Config = {
    ...
    ,setData: function(newData){
      ...
      //插入排序
      for( var q = 0; q < this.myData.length; ++q){
        for( var t = q+1; t < this.myData.length; ++t){
          if( parseInt(this.myData[t].time, 10) < parseInt(this.myData[q].time, 10) ){
            var m = this.myData[q];
            this.myData[q] = this.myData[t];
            this.myData[t] = m;
          }
        }
      }
...
KISSY.add('gallery/ks-timeline/1.0/control', function(S, Base, BG, Toolbar, MainContent, DD, TrackConfig){
  var $ = S.all;
...
KISSY.add('gallery/ks-timeline/1.0/index', function(S, Control){
  var $ = S.all;
  //class
  /**
    * @param{config} 
